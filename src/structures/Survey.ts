        ...
        if(channel instanceof TextChannel){
            ...
            if(message instanceof Message){
                ...
                const embed = new MessageEmbed()
                    ...
                    .setTimestamp()
                    .setTitle(this.title)
                    .addField(`<a:yes:${Constants.AGREE_EMOJI_ID}> (${server.translate('global.yes')})`, agreeCount, true)
                    .addField(`<a:no:${Constants.DISAGREE_EMOJI_ID}> (${server.translate('global.no')})`, disagreeCount, true)

                await message.delete({ timeout: 0 })
                await message.channel.send(`${Constants.RUBY_EMOJI} **${server.translate('structures.survey.results.survey')}**`, {
                    embed
                })
            }
        }
